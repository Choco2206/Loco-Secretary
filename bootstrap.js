...
function fc27Load() {
  try {
    ...
    return d;
  } catch (e) { console.error('[fc27] load', e); return { players: {} }; }
}
...
function fc27Menu(id, placeholder, exclude = []) {
  return new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId(id).setPlaceholder(placeholder).setMinValues(1).setMaxValues(1).addOptions(fc27Options(exclude)));
}
function fc27PublicButtons() { return new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('fc27_start').setLabel('Positionen auswählen').setEmoji('📍').setStyle(ButtonStyle.Primary)); }
function fc27AdminButtons() { return new ActionRowBuilder().addComponents(
...
function fc27UserMenu(id, placeholder) { return new ActionRowBuilder().addComponents(new UserSelectMenuBuilder().setCustomId(id).setPlaceholder(placeholder).setMinValues(1).setMaxValues(1)); }

async function fc27Members(guild) {
  await guild.members.fetch();
  return [...guild.members.cache.values()].filter(m => !m.user.bot && m.roles.cache.has(CONFIG.roles.locoSquad)).sort((a,b) => a.displayName.localeCompare(b.displayName, 'de'));
}
async function fc27StatusText(guild) {
  const data = fc27Load(); const members = await fc27Members(guild);
  let done = 0;
  const lines = members.map(m => {
    const p = data.players[m.id]; const status = p?.excluded ? '❌' : p?.self?.length === 3 ? '✅' : '⬜';
    if (status !== '⬜') done++;
    return '<@' + m.id + '>  ' + status;
  });
...
async function fc27EnsurePublic() {
  const ch = getChannel(FC27_PUBLIC_CHANNEL); if (!ch?.isTextBased()) return;
  const s = await fc27StatusText(ch.guild);
  const embed = new EmbedBuilder().setColor(CONFIG.branding.accentColor).setTitle('🐺 FC 27 – Kaderplanung').setDescription(
...
async function fc27EnsureAdmin() {
  const ch = getChannel(FC27_ADMIN_CHANNEL); if (!ch?.isTextBased()) return;
  const data = fc27Load(); const members = await fc27Members(ch.guild);
  const lines = members.map(m => { const p=data.players[m.id]; return '<@'+m.id+'>  Spieler: '+(p?.self?.length===3?'✅':'⬜')+' | Choko: '+(p?.manager?.length===3?'✅':'⬜')+(p?.excluded?' | ❌ Nicht eingeplant':''); });
  const embed = new EmbedBuilder().setColor(CONFIG.branding.accentColor).setTitle('🔒 FC 27 – Kaderplanung intern').setDescription('Hier hinterlegst du deine Einschätzung unabhängig von der Spielerwahl.\n\n'+(lines.join('\n')||'Keine Spieler gefunden.')).setFooter({ text: CONFIG.branding.footer });
  const msgs = await ch.messages.fetch({ limit: 30 });
  const old = [...msgs.values()].find(m => m.author.id === client.user.id && m.embeds?.[0]?.title === '🔒 FC 27 – Kaderplanung intern');
  const payload={embeds:[embed],components:[fc27AdminButtons()]}; if(old) await old.edit(payload); else await ch.send(payload);
}
async function fc27Refresh(){ await fc27EnsurePublic(); await fc27EnsureAdmin(); }
function fc27ResultText(guild) {
  const data=fc27Load(); const rows=[];
...
client.once(Events.ClientReady, async () => { setTimeout(() => fc27Refresh().catch(console.error), 2500); });
...
client.on(Events.InteractionCreate, async interaction => {
  try {
    if (interaction.isButton() && interaction.customId === 'fc27_start') {
      ...
      const d=fc27Load(); if(d.players[interaction.user.id]?.excluded) return interaction.reply({content:'Du bist aktuell für diese Kaderplanung als nicht eingeplant markiert.',flags:MessageFlags.Ephemeral});
    ...
    if(interaction.isStringSelectMenu() && interaction.customId==='fc27_self_hp') {
      const hp=interaction.values[0]; return interaction.update({content:'**2/3 – Nebenposition**\nDeine zweite Wahl.',components:[fc27Menu('fc27_self_np1:'+hp,'Nebenposition wählen',[hp])]});
    }
    if(interaction.isStringSelectMenu() && interaction.customId.startsWith('fc27_self_np1:')) {
      const hp=interaction.customId.split(':')[1], np1=interaction.values[0]; return interaction.update({content:'**3/3 – 2. Nebenposition**\nDeine dritte Wahl.',components:[fc27Menu('fc27_self_np2:'+hp+':'+np1,'2. Nebenposition wählen',[hp,np1])]});
    }
    if(interaction.isStringSelectMenu() && interaction.customId.startsWith('fc27_self_np2:')) {
      const [,hp,np1]=interaction.customId.split(':'); const np2=interaction.values[0]; const d=fc27Load(); d.players[interaction.user.id] ||= {}; if(d.players[interaction.user.id].excluded) return interaction.update({content:'Du bist aktuell nicht eingeplant.',components:[]}); d.players[interaction.user.id].self=[hp,np1,np2]; fc27Save(d); await interaction.update({content:'✅ Gespeichert: **'+hp+' → '+np1+' → '+np2+'**\nDu kannst deine Auswahl über den Button jederzeit ändern.',components:[]}); await fc27Refresh(); return;
    }
    if(interaction.isUserSelectMenu() && interaction.customId.startsWith('fc27_admin_user_')) {
      ...
      const id=interaction.values[0], d=fc27Load(); d.players[id] ||= {};
      if(interaction.customId==='fc27_admin_user_exclude'){ d.players[id].excluded=true; fc27Save(d); await interaction.update({content:'❌ <@'+id+'> wird aktuell nicht eingeplant.',components:[]}); await fc27Refresh(); return; }
      if(interaction.customId==='fc27_admin_user_restore'){ d.players[id].excluded=false; fc27Save(d); await interaction.update({content:'↩ <@'+id+'> wurde wieder aufgenommen.',components:[]}); await fc27Refresh(); return; }
      if(interaction.customId==='fc27_admin_user_assess'){ if(d.players[id].excluded) return interaction.update({content:'Der Spieler ist aktuell als nicht eingeplant markiert.',components:[]}); return interaction.update({content:'**1/3 – Deine Hauptpositions-Einschätzung für <@'+id+'>**',components:[fc27Menu('fc27_mgr_hp:'+id,'Hauptposition einschätzen')]}); }
    }
    if(interaction.isStringSelectMenu() && interaction.customId.startsWith('fc27_mgr_hp:')) { const id=interaction.customId.split(':')[1], hp=interaction.values[0]; return interaction.update({content:'**2/3 – Deine Nebenpositions-Einschätzung**',components:[fc27Menu('fc27_mgr_np1:'+id+':'+hp,'Nebenposition einschätzen',[hp])]}); }
    if(interaction.isStringSelectMenu() && interaction.customId.startsWith('fc27_mgr_np1:')) { const [,id,hp]=interaction.customId.split(':'); const np1=interaction.values[0]; return interaction.update({content:'**3/3 – Deine 2. Nebenpositions-Einschätzung**',components:[fc27Menu('fc27_mgr_np2:'+id+':'+hp+':'+np1,'2. Nebenposition einschätzen',[hp,np1])]}); }
    if(interaction.isStringSelectMenu() && interaction.customId.startsWith('fc27_mgr_np2:')) { const [,id,hp,np1]=interaction.customId.split(':'); const np2=interaction.values[0], d=fc27Load(); d.players[id] ||= {}; d.players[id].manager=[hp,np1,np2]; fc27Save(d); await interaction.update({content:'✅ Deine Einschätzung für <@'+id+'>: **'+hp+' → '+np1+' → '+np2+'**',components:[]}); await fc27Refresh(); return; }
  } catch(e) { console.error('[fc27 interaction]',e); }
});
`;
